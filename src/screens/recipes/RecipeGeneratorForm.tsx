...
export type RecipeGeneratorFormProps = {
  headerTitle: string;
  subtitle: string;
  lockedMealType?: MealType;
  primaryLabel: string;
  primaryIcon: keyof typeof Ionicons.glyphMap;
  // Persist / attach the preview. The host screen owns navigation on success.
  onPrimary: (preview: GeneratedRecipe, mealType: MealType | null) => Promise<void>;
};

// Shared AI recipe generator: pantry-aware form, generate + refine loop, and the
// preview card. Persisting the result is delegated to `onPrimary` so the same UI
// can save to the library (Recipes page) or attach to a meal plan (planner).
export default function RecipeGeneratorForm({
  ...
  const [mealType, setMealType] = useState<MealType | null>(lockedMealType ?? null);
  ...
  const lockedMeal = lockedMealType ? MEALS.find((m) => m.type === lockedMealType) : null;
  ...
  const handlePrimary = async () => {
    ...
    try {
      setSubmitting(true);
      await onPrimary(preview, mealType);
    } catch {
      showFeedback(`Unable to ${primaryLabel.toLowerCase()}`, 'Please try again.', 'error');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        {/* Meal type — a locked badge in the planner, a selector on the recipes page */}
        <Text style={styles.label}>Meal type</Text>
        {lockedMeal ? (
          <View style={styles.lockedRow}>
            <View style={[styles.mealChip, styles.mealChipSelected]}>
              <Ionicons name={lockedMeal.icon} size={16} color="#fff" />
              <Text style={[styles.mealChipText, styles.mealChipTextSelected]}>
                {lockedMeal.label}
              </Text>
            </View>
          </View>
        ...
        {preview ? (
          ...
          >
            ...
            <View style={styles.previewActions}>
              <TouchableOpacity
                style={[styles.primaryBtn, styles.flex1, submitting && styles.disabled]}
                activeOpacity={0.85}
                disabled={submitting}
...
const styles = StyleSheet.create({
  ...
  lockedRow: { flexDirection: 'row', marginBottom: 20 },
